...
def simulate(dt, T, x_init, x_target, plan_length, control_func, world, path_rrt, waypoint_radius, num_states=6,
             num_inputs=3):
    ## Timesteps
    ...
    last_plan = None
    last_input = None
    target_state = [path_rrt.path_points[waypoint_idx][0], path_rrt.path_points[waypoint_idx][1],
    ...
    for t in range(len(timesteps)):
        ...
        distance_2_next_wp = np.sqrt((current_state[0] - path_rrt.path_points[waypoint_idx][0]) ** 2 + (
                    current_state[1] - path_rrt.path_points[waypoint_idx][1]) ** 2)
